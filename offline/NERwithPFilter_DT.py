...
import openpyxl
import re
from PrototypeGeneration import Proto_Gen_missMISC, Proto_Gen_missTarget, Proto_Gen_wrongclass
import numpy as np
...
def NER_PF(llm, llm_name, type, sheet, LD):
    label = type[:3].upper()
    ...
    prototype_for_missMISC_right, prototype_for_missMISC_right_disp, prototype_for_missMISC_right_meandist, prototype_for_missMISC_right_cov, prototype_for_missMISC_wrong, prototype_for_missMISC_wrong_disp, prototype_for_missMISC_wrong_meandist, prototype_for_missMISC_wrong_cov = Proto_Gen_missMISC(
        llm_name, type)
    prototype_for_wrongclass_right, prototype_for_wrongclass_right_disp, prototype_for_wrongclass_right_meandist, prototype_for_wrongclass_right_cov,prototype_for_wrongclass_wrong, prototype_for_wrongclass_wrong_disp, prototype_for_wrongclass_wrong_meandist, prototype_for_wrongclass_wrong_cov = Proto_Gen_wrongclass(llm_name, type)
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        sentence：{sentence}
                """
        prompt = ChatPromptTemplate.from_template(template0)
        output_parser = CommaSeparatedListOutputParser()
        chain = prompt | llm | output_parser
        res0 = chain.invoke({"type": type, "sentence": cell_sentence}).replace('"', '').replace("'", "").replace("\n",
                                                                                                                "").replace(
            "* ", "").replace("*", "").replace(".", "")
        ...
        if 'This ' not in res0 and 'There ' not in res0 and res0 != '' and ' no ' not in res0:
            ...
            res1 = chain1.invoke(
                {"Entity_list": str(v_predict_entity_list), "sentence": cell_sentence, "label_descri": LD,
            ...
            if len(predict_entity_list) > 0:
                for i in range(len(predict_entity_list)):
                    if predict_entity_list[i] not in predict_MISC_entity_list:
                        ...
                        if Filter_for_wrongclass([float(predict_rating_list[i]), float(predict_MISC_rating_list[predict_MISC_entity_list.index(predict_entity_list[i])])],
                                                 prototype_for_wrongclass_right, prototype_for_wrongclass_right_disp, prototype_for_wrongclass_right_meandist, prototype_for_wrongclass_right_cov,prototype_for_wrongclass_wrong, prototype_for_wrongclass_wrong_disp, prototype_for_wrongclass_wrong_meandist, prototype_for_wrongclass_wrong_cov):
                            if predict_entity_list[i] in final_prediction_list:
                                final_prediction_list.remove(predict_entity_list[i])
...
if __name__ == "__main__":
    ...
    LD_list = ["This category includes names of persons, such as individual people or groups of people with personal names.","This category includes names of formally structured groups, such as companies, institutions, agencies, or teams, within text.","This category includes names of specific geographical places, such as cities, countries, regions, or landmarks, within text."]
    ...
    for dataset in dataset_list:
        ...
        for llm_name in llm_list:
            ...
            for i in range(len(type_list)):
                result = NER_PF(llm, llm_name, type_list[i], sheet, LD_list[i])
                with open(f"../Result_pf/{dataset}/{type_list[i]}/{llm_name}.txt", "w", encoding="utf-8") as f:
                    f.write(result)
