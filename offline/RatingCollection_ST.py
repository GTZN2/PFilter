...
def RatingCollect(llm, type, sheet,LD):
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            res = chain.invoke({"label_descri":LD,"type":type,"sentence": cell_sentence}).replace('"', '').replace("'", "").replace("\n", "").replace("* ", "").replace("*", "").replace(".", "")


            print(res)

            if process_entity_string(res) != "":
                predict_entity_list,predict_rating_list,predict_MISC_entity_list,predict_MISC_rating_list = process_entity_string(res)
            ...
            rating_sent = process_entity_lists(predict_entity_list, predict_MISC_entity_list, predict_rating_list, predict_MISC_rating_list, true_entity_list)
