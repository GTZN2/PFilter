    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        payload = json.dumps({
            ...
            "messages": [
                {
                    "role": "user",
                    "content": template.format(sentence=cell_sentence, type=type)
                }
            ]
        })
        res = requests.request("POST", url, headers=headers, data=payload).json()['choices'][0]['message']['content'].replace('"', '').replace("'", "").replace("\n", "").replace("* ", "").replace("*", "").replace(
            ".", "")
        ...
        if 'This ' not in res and 'There ' not in res and res != '' and ' no ' not in res:
            ...
            head_wrong_sent, inside_wrong_sent, wrong_class_sent, miss_sent = NEREval(predict_entity_list,
                                                                                      true_entity_list)
...
if __name__ == "__main__":
    ...
    for dataset in dataset_list:
        ...
        for i in range(len(type_list)):
            result = NER(type_list[i], sheet, LD_list[i])
            with open(f"../Result_v/{dataset}/{type_list[i]}/GPT3.5.txt", "w", encoding="utf-8") as f:
